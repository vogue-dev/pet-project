...
export default class Clicker extends React.Component {
    ...
    incrementUpgrade = () => {
        if (this.state.handleClick === 1 && this.state.clickPoints >= this.state.handleClickUpgrade)
            this.setState({ handleClick: 2, clickPoints: this.state.clickPoints - this.state.handleClickUpgrade })
    }
    ...
    tick = () => {
        ...
        this.state.machineClickerQuantity >= 1 ? 
            this.setState({ clickPoints: this.state.clickPoints + 3 * this.state.machineClickerQuantity }) :
            this.setState({ clickPoints: this.state.clickPoints })
        this.state.factoryClickerQuantity >= 1 ? 
            this.setState({ clickPoints: this.state.clickPoints + 12 * this.state.factoryClickerQuantity }) :
            this.setState({ clickPoints: this.state.clickPoints })
    }
                    ...
                    <div className="clicker-section">
                        <div className="clicker__game">
                            <div className="click__points">
                                ...
                                <div className="points-per-sec">(per second: +{0.5 * this.state.autoBotQuantity + (3 * this.state.machineClickerQuantity)+ (12 * this.state.factoryClickerQuantity)})</div>
                            </div>
                            <div className="increment">
                                <div className="increment-click" onClick={ this.increment }>+{this.state.handleClick} cp/click</div>
                                {
                                    this.state.handleClick === 1 && this.state.clickPoints >= this.state.handleClickUpgrade ? 
                                        <div className="increment-click-upgrade active" onClick={ this.incrementUpgrade }>LVL UP x2 ({this.state.handleClickUpgrade}cp)</div> :
                                        <div className="increment-click-upgrade" onClick={ this.incrementUpgrade }>LVL UP x2 ({this.state.handleClickUpgrade}cp)</div>
                                }
                            </div>
                            {
                            ...
                            { 
                                ...
                                <ClickBlock 
                                    className={ "btn auto-clicker active" } 
